...
#include "ns3/qkd-v6-packet-filter.h"
...
NS_LOG_COMPONENT_DEFINE ("QKDv6Manager");

NS_OBJECT_ENSURE_REGISTERED (QKDv6Manager);

TypeId QKDv6Manager::GetTypeId (void) 
{
  static TypeId tid = TypeId ("ns3::QKDv6Manager")
    .SetParent<Object> ()
    .AddConstructor<QKDv6Manager> ()
    .AddAttribute ("BufferList", "The list of buffers associated to this Manager.",
                   ObjectVectorValue (),
                   MakeObjectVectorAccessor (&QKDv6Manager::m_buffers),
                   MakeObjectVectorChecker<QKDv6Manager> ())
    ;
  return tid;
} 
QKDv6Manager::QKDv6Manager () 
{ 
...
void 
QKDv6Manager::DoInitialize (void)
{
    NS_LOG_FUNCTION (this); 
    Object::DoInitialize ();
}
...
void 
QKDv6Manager::DoDispose ()
{
    NS_LOG_FUNCTION (this); 
    for (std::vector<Ptr<QKDv6Buffer> >::iterator i = m_buffers.begin ();
       i != m_buffers.end (); i++)
    {
      Ptr<QKDv6Buffer> device = *i;
      device->Dispose ();
      *i = 0;
    }
    m_buffers.clear (); 

    std::map<Address, QKDv6Manager::Connection >::iterator j; 
    for (j = m_destinations.begin (); !(j == m_destinations.end ()); j++){ 
        j->second.QKDv6NetDeviceSrc = 0;
        j->second.QKDv6NetDeviceDst = 0;
...
void
QKDv6Manager::UseRealStorages(const bool& useRealStorages){
    m_useRealStorages = useRealStorages;
}

void
QKDv6Manager::AddNewLink (  
    Ptr<QKDv6NetDevice>       QKDv6NetDeviceSrc,
    Ptr<QKDv6NetDevice>       QKDv6NetDeviceDst,
    Ptr<NetDevice>          IPNetDeviceSrc,
    Ptr<NetDevice>          IPNetDeviceDst,
    Ptr<QKDv6Crypto>          Crypto,
    Ptr<Socket>             socket,
    Ptr<Socket>             socketSink,
    std::string             socketType,
    uint32_t                underlayPortNumber, 
    Ipv6InterfaceAddress    IPQKDSrc,   //QKD IP Src Address - overlay device
    Ipv6InterfaceAddress    IPQKDDst,   //QKD IP Dst Address - overlay device 
    Ipv6InterfaceAddress    IPSrc,      //IP Src Address - underlay device
    Ipv6InterfaceAddress    IPDst,      //IP Dst Address - underlay device 
    bool                    isMaster,  
    uint32_t                Mmin, 
    uint32_t                Mthr, 
    uint32_t                Mmax, 
...
{  
    ...
    NS_ASSERT (IPQKDSrc.GetPrefix () == IPQKDDst.GetPrefix ());

    Ptr<Node> a = IPNetDeviceSrc->GetNode();
    Ptr<Node> b = IPNetDeviceDst->GetNode();
    
    struct QKDv6Manager::Connection newConnection; 

    newConnection.QKDv6NetDeviceSrc = QKDv6NetDeviceSrc;
    newConnection.QKDv6NetDeviceDst = QKDv6NetDeviceDst;
    ...
    /*
    std::map<Address, QKDv6Manager::Connection >::iterator i = FetchConnection ( IPNetDeviceSrc.GetAddress() );
    ...
    */
    ...
    newConnection.buffer = CreateObject<QKDv6Buffer> (a->GetId(), b->GetId(), m_useRealStorages );
    ...
    NS_LOG_FUNCTION(this << "QKDv6NetDeviceSrc:" << newConnection.QKDv6NetDeviceSrc);
    if(newConnection.QKDv6NetDeviceSrc != 0){
        NS_LOG_FUNCTION(this << "QKDv6NetDeviceSrc address:" << newConnection.QKDv6NetDeviceSrc->GetAddress() );
    }

    NS_LOG_FUNCTION(this << "QKDv6NetDeviceDst:" << newConnection.QKDv6NetDeviceDst);
    if(newConnection.QKDv6NetDeviceDst != 0){
        NS_LOG_FUNCTION(this << "QKDv6NetDeviceDst address:" << newConnection.QKDv6NetDeviceDst->GetAddress() );
    }
...
uint32_t 
QKDv6Manager::GetNumberOfDestinations(){
    return m_destinations.size();
}

std::vector<QKDv6Manager::addressGroup> 
QKDv6Manager::GetMapOfSourceDestinationAddresses(){
    ...
    for (j = m_destinations.begin (); !(j == m_destinations.end ()); j++){ 
        ...
        struct QKDv6Manager::addressGroup detail;  

        //If only underlay network
        if( j->second.QKDv6NetDeviceSrc == 0){ 

            NS_LOG_FUNCTION (this << "\t Destination address found:" << j->second.IPSrc.GetAddress() );
        
            detail.destinationNode = j->second.IPNetDeviceDst->GetNode();
            detail.sourceAddress = j->second.IPSrc.GetAddress();
            detail.destinationAddress = j->second.IPDst.GetAddress();
            m_links.push_back(detail);

        //If overlay network
        }else{ 

            NS_LOG_FUNCTION (this << "\t Destination address found:" << j->second.IPQKDDst.GetAddress() );

            detail.destinationNode = j->second.QKDv6NetDeviceDst->GetNode();
            detail.sourceAddress = j->second.IPQKDSrc.GetAddress();
            detail.destinationAddress = j->second.IPQKDDst.GetAddress();
            m_links.push_back(detail);
...
QKDv6Manager::PopulateLinkStatusesForNeighbors(
    ...
    Ipv6Address bestOutput;     
    return bestOutput;
}

uint32_t
QKDv6Manager::FetchLinkThresholdHelpValue(){
...
void        
QKDv6Manager::CalculateLinkThresholdHelpValue(){

    NS_LOG_FUNCTION(this << m_linksThresholdHelpValue);
}

void
QKDv6Manager::SetLinkThresholdValue( const uint32_t& proposedLaValue, const Address sourceAddress ){

    NS_LOG_FUNCTION (this << proposedLaValue << sourceAddress);
}

uint32_t
QKDv6Manager::GetLinkThresholdValue(const Address sourceAddress ){

    NS_LOG_FUNCTION (this << sourceAddress);
    return 0;
}

std::map<Address, QKDv6Manager::Connection >::iterator
QKDv6Manager::FetchConnection(const Address sourceAddress){

    NS_LOG_FUNCTION (this << sourceAddress << m_destinations.size() );

    std::map<Address, QKDv6Manager::Connection >::iterator i = m_destinations.find (sourceAddress);
    if (i == m_destinations.end ()){
        std::map<Address, QKDv6Manager::Connection >::iterator j;
        for (j = m_destinations.begin (); !(j == m_destinations.end ()); j++){ 
            if (
                 (j->second.IPNetDeviceSrc != 0 && j->second.IPNetDeviceSrc->GetAddress() == sourceAddress) || 
                 (j->second.QKDv6NetDeviceSrc != 0 && j->second.QKDv6NetDeviceSrc->GetAddress() == sourceAddress)
             ) {
                return j;
            }
        }
...
Ptr<NetDevice>
QKDv6Manager::GetSourceNetDevice (const Address address){
    ...
    for (j = m_destinations.begin (); !(j == m_destinations.end ()); j++){ 
        ...
        //if only underlay network
        if( j->second.QKDv6NetDeviceSrc == 0 && j->second.IPNetDeviceSrc->GetAddress() == address){ 
            NS_LOG_FUNCTION (this << "\t Destination address found:" << j->second.IPNetDeviceSrc->GetAddress() );
            NS_LOG_FUNCTION (this << "Src" << j->second.IPSrc.GetAddress() << "Dst" << j->second.IPDst.GetAddress() );
            return j->second.IPNetDeviceSrc;

        //if overlay network
        }else if( j->second.QKDv6NetDeviceSrc != 0 && j->second.QKDv6NetDeviceSrc->GetAddress() == address){ 
            NS_LOG_FUNCTION (this << "\t Destination address found:" << j->second.QKDv6NetDeviceSrc->GetAddress() ); 
            NS_LOG_FUNCTION (this << "Src" << j->second.IPQKDSrc.GetAddress() << "Dst" << j->second.IPQKDDst.GetAddress() );
            return j->second.QKDv6NetDeviceSrc;
        }
    }
    return 0;
}
...
bool
QKDv6Manager::VirtualSendOverlay (Ptr<Packet> p, const Address& source, const Address& dst, uint16_t protocolNumber)
{
    return VirtualSend (p, source, dst, protocolNumber, 0);
}

bool
QKDv6Manager::VirtualSend (
    Ptr<Packet> p, 
    ...
    /*
    std::cout << "--------QKDv6Manager::VirtualSend-----------\n";   
    std::cout << p->GetUid() << "\t" << p->GetSize() << "\n";
    p->Print(std::cout);
    std::cout << "\n\n";
    std::cout << "--------End of QKDv6Manager::VirtualSend-----------\n";
    */
...
QKDv6Manager::ForwardToSocket (
    ...
    //if unicast delivery
    std::map<Address, QKDv6Manager::Connection >::iterator i = FetchConnection (source);
    if (i != m_destinations.end ()){

        /**
        *   Since some routing protocols might not be aware of QKDv6buffers and QKD materials in these buffers, 
        ...
        */
        ...
        //if only underlay network
        if( i->second.QKDv6NetDeviceSrc == 0){


            NS_LOG_DEBUG (this 
                << "Sending UNICAST to socket\t" << i->second.socket << "\t"  
                << "IPQKDSrc:\t" << i->second.IPQKDSrc.GetAddress() << "\t" 
                << "IPQKDDst:\t" << i->second.IPQKDDst.GetAddress() << "\t" 
                << "IPNetDeviceSrc:\t" << i->second.IPNetDeviceSrc->GetAddress() << "\t" 
                << "IPNetDeviceDst:\t" << i->second.IPNetDeviceDst->GetAddress() << "\t" 
                << "IPSrc:\t" << i->second.IPSrc.GetAddress() << "\t" 
                << "IPDst:\t" << i->second.IPDst.GetAddress() << "\t" 
            ...
            NS_LOG_DEBUG (this << "SINGLE TCP/IP Network" << i->second.QKDv6NetDeviceSrc);
        ...
        //if overlay network
            ...
            NS_LOG_FUNCTION (this << "OVERLAY Network" << i->second.QKDv6NetDeviceSrc);
...
void 
QKDv6Manager::VirtualReceiveSimpleNetwork (Ptr<NetDevice> device, Ptr<const Packet> packet,
            ...
            )
{
    ...
    std::map<Address, QKDv6Manager::Connection >::iterator i = FetchConnection (device->GetAddress());
        ...
        if(i->second.IPNetDeviceSrc == device){
            ...
            for( ; it != processedPackets.end(); ++it){
                ...
                if(p != 0){
                    ...
                    /*
                    std::cout << "--------QKDv6Manager::VirtualReceiveSimpleNetwork-----------\n";   
                    std::cout << p->GetUid() << "\t" << p->GetSize() << "\n";
                    p->Print(std::cout);
                    std::cout << "\n\n";
                    std::cout << "--------End of QKDv6Manager::VirtualReceiveSimpleNetwork-----------\n";
                    */
...
void 
QKDv6Manager::VirtualReceive (Ptr<Socket> socket)
{       
    ...
    for (i = m_destinations.begin (); !(i == m_destinations.end ()); i++){        
        ...
        if( sNetDevice != 0 && sSinkNetDevice != 0 && sNetDevice->GetAddress() == sSinkNetDevice->GetAddress() ){
            NS_LOG_FUNCTION (this << "QKDv6NetDevice Found!");
            break;
        } 
    }
    ...
    if( i != m_destinations.end () ){
        ...
        if(i->second.socketType == "ns3::TcpSocketFactory"){
            ...
            while ((packet = socket->RecvFrom (from)))
            {
                ...
                //PERFORM DECRYPTION and AUTHENTICATION-CHECK
                std::vector<Ptr<Packet> > processedPackets = ProcessIncomingRequest (i->second.QKDv6NetDeviceSrc, packet);
                ...
                for( ; it != processedPackets.end(); ++it){
                    ...
                    Ptr<Packet> p = *it;
                    if (p != 0){
                        i->second.QKDv6NetDeviceSrc->Receive (
                            p, 
                            0x0800, 
                            i->second.QKDv6NetDeviceDst->GetAddress (), //who is sending packet
                            i->second.QKDv6NetDeviceSrc->GetAddress (), //who is receiving packet
        ...
        //////////////////////////////////UDP SOCKET/////////////////////////////////////////
            ...
            for( ; it != processedPackets.end(); ++it){
                ...
                NS_LOG_FUNCTION (this << "Delivering packet " << p << " to " << i->second.QKDv6NetDeviceSrc);
    ...
    /*
    std::cout << "--------QKDv6Manager::VirtualReceive-----------\n";   
    std::cout << packet->GetUid() << "\t" << packet->GetSize() << "\n";
    packet->Print(std::cout);
    std::cout << "\n\n";
    std::cout << "--------End of QKDv6Manager::VirtualReceive-----------\n";   
    */ 
}
 
void 
QKDv6Manager::HandleAccept (Ptr<Socket> s, const Address& from)
{
  NS_LOG_FUNCTION (this << s << from);
  s->SetRecvCallback (MakeCallback (&QKDv6Manager::VirtualReceive, this)); 
}

void 
QKDv6Manager::ConnectionSucceeded (Ptr<Socket> socket)
{
    NS_LOG_FUNCTION (this << socket);
    NS_LOG_FUNCTION (this << "QKDv6Manager Connection succeeded");
//    m_connected = true; 
}

void 
QKDv6Manager::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  NS_LOG_FUNCTION (this << "QKDv6Manager, Connection Failed");
}

Ptr<Node> 
QKDv6Manager::GetDestinationNode (const Address sourceAddress) 
{
    NS_LOG_FUNCTION (this << sourceAddress);
    std::map<Address, QKDv6Manager::Connection >::iterator i = FetchConnection (sourceAddress);
    if (i != m_destinations.end ())
        return i->second.IPNetDeviceDst->GetNode (); 
    return 0;
}

QKDv6Manager::Connection
QKDv6Manager::GetConnectionDetails (const Address sourceAddress)
{
    NS_LOG_FUNCTION (this << sourceAddress);
    std::map<Address, QKDv6Manager::Connection >::iterator i = FetchConnection (sourceAddress);
    if (i != m_destinations.end ())
...
QKDv6Manager::Connection
QKDv6Manager::GetConnectionDetails (const uint32_t& bufferId)
{
...
uint32_t 
QKDv6Manager::GetBufferPosition (const Address& sourceAddress)
{
    NS_LOG_FUNCTION (this << sourceAddress);

    Ptr<QKDv6Buffer> tempBuffer;  
    
    std::map<Address, QKDv6Manager::Connection >::iterator i = FetchConnection (sourceAddress);
    if (i != m_destinations.end ()){ 
        tempBuffer = i->second.buffer;
        uint32_t a = 0;
        for (std::vector<Ptr<QKDv6Buffer> >::iterator i = m_buffers.begin (); !(i == m_buffers.end ()); i++){
            if( **i == *tempBuffer ){
                return a;
            } 
            a++;
...
Ptr<QKDv6Buffer>
QKDv6Manager::GetBufferBySourceAddress (const Address& sourceAddress)
{
    NS_LOG_FUNCTION (this << sourceAddress); 
...
Ptr<QKDv6Buffer>
QKDv6Manager::GetBufferByBufferPosition (const uint32_t& bufferPosition)
{
...
uint32_t 
QKDv6Manager::GetNBuffers (void) const
{
    NS_LOG_FUNCTION (this);
...
uint8_t 
QKDv6Manager::FetchStatusForDestinationBuffer(Ptr<NetDevice> dev)
{   
    ...
    for (i = m_destinations.begin (); !(i == m_destinations.end ()); i++){
        ...
        if( (i->second.QKDv6NetDeviceSrc == 0 && i->second.IPNetDeviceSrc == dev) ||
            i->second.QKDv6NetDeviceSrc == dev
        ){  
...
QKDv6Manager::ProcessIncomingRequest (Ptr<NetDevice> dev, Ptr<Packet> p)
{
...
std::vector<Ptr<Packet> >
QKDv6Manager::ProcessOutgoingRequest (Ptr<NetDevice> dev, Ptr<Packet> p)
{
    ...
    if(p->PeekPacketTag(tag)) {
        ...
        NS_LOG_FUNCTION ( 
            this << "No QKDv6InternalTag detected!" 
...
double
QKDv6Manager::FetchPublicChannelPerformance (Ipv6Address nextHop){
...
uint32_t
QKDv6Manager::FetchMaxNumberOfRecordedKeyChargingTimePeriods (Ipv6Address nextHop){
...
void
QKDv6Manager::UpdatePublicChannelMetric(const Address sourceAddress){
...
void
QKDv6Manager::UpdateQuantumChannelMetric(const Address sourceAddress){

    NS_LOG_FUNCTION (this << sourceAddress);
}

bool
QKDv6Manager::AddNewKeyMaterial (const Address sourceAddress, uint32_t& newKey)
{
...
bool
QKDv6Manager::IsMarkedAsEncrypt (Ptr<Packet> p)
{
...
Ptr<Packet> 
QKDv6Manager::MarkEncrypt (Ptr<Packet> p)
{
    NS_LOG_FUNCTION (this << p->GetUid() << p->GetSize() ); 

    //default values are OTP and VMAC
    return MarkEncrypt(p, QKDv6CRYPTO_OTP, QKDv6CRYPTO_AUTH_VMAC);  
}

Ptr<Packet> 
QKDv6Manager::MarkMaxDelay (Ptr<Packet> p,uint32_t delay)
{
...
Ptr<Packet> 
QKDv6Manager::MarkAuthenticate (Ptr<Packet> p)
{
    NS_LOG_FUNCTION (this << p->GetUid() << p->GetSize() ); 
    return MarkEncrypt(p, 0, QKDv6CRYPTO_AUTH_VMAC);  
}

Ptr<Packet> 
QKDv6Manager::MarkEncrypt (Ptr<Packet> p, uint8_t encryptionType, uint8_t authneticationType)
{
...
bool
QKDv6Manager::CheckForResourcesToProcessThePacket(
    Ptr<Packet> p,
...
uint32_t
QKDv6Manager::FetchPacketTos(Ptr<Packet> p){
    ...
    QKDv6InternalTOSTag temp_qkdNextHopTag; 
    ...
    if ( p->PeekPacketTag (ipTosTag) )
    {
        ...
        NS_LOG_WARN(this << "\t" << "Found TOS tag using QKDv6InternalTOSTag; value:" << "\t" << (uint32_t) tos);
...
uint32_t
QKDv6Manager::TosToBand(const uint32_t& tos){

    NS_LOG_FUNCTION( this << "\t" << tos );

    Ptr<PfifoFastQKDv6PacketFilter> filter = CreateObject<PfifoFastQKDv6PacketFilter> ();
    ...
    */
    ...
    /*
     * PfifoFastQKDv6PacketFilter is the filter to be added to the PfifoFast
