...
#ifndef QKD_V6_GRAPH_H
#define QKD_V6_GRAPH_H
 ...
 */
class QKDv6Graph : public Object 
{
public:
    ...
    */
    ...
    /**
    * \brief MCurrent value of the QKDv6Buffer changed, so plot it on the graph
    */
	void ProcessMCurrent(uint32_t value);

    /**
    * \brief The status of the QKDv6Buffer changed, so plot it on the graph
    */
	void ProcessMStatus(uint32_t value); 

    /**
    * \brief The Mthr value of the QKDv6Buffer changed, so plot it on the graph
    */
...
private: 
	...
	Ptr<QKDv6Buffer>		buffer;	//!< QKDv6Buffer associated with the QKDv6Graph
