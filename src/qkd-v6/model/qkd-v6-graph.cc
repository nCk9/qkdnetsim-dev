...
NS_LOG_COMPONENT_DEFINE ("QKDv6Graph");

NS_OBJECT_ENSURE_REGISTERED (QKDv6Graph);

TypeId QKDv6Graph::GetTypeId (void) 
{
  static TypeId tid = TypeId ("ns3::QKDv6Graph")
    .SetParent<Object> () 
    ; 
  return tid;
}
 
QKDv6Graph::~QKDv6Graph(){}
...
{ 	 
	...
	Ptr<QKDv6Manager> manager = node->GetObject<QKDv6Manager> ();
	this->buffer = manager->GetBufferByBufferPosition (bufferPosition); 

	QKDv6Manager::Connection connection = manager->GetConnectionDetails(this->buffer->GetBufferId() );
...
void
QKDv6Graph::InitTotalGraph() const{
...
void
QKDv6Graph::ProcessMCurrent(uint32_t newValue){ 
...
void
QKDv6Graph::ProcessMThrStatus(uint32_t newValue){ 
...
void
QKDv6Graph::ProcessMStatusHelpFunction(double time, uint32_t newValue){
...
void
QKDv6Graph::ProcessMStatus(uint32_t newValue){ 
