...
NS_LOG_COMPONENT_DEFINE ("QKDv6Crypto"); 

NS_OBJECT_ENSURE_REGISTERED (QKDv6Crypto);
...
std::string
QKDv6Crypto::VectorToString(std::vector<uint8_t> inputVector)
{
...
std::vector<uint8_t>
QKDv6Crypto::QKDv6HeaderToVector(QKDv6Header& qkdv6Header)
{
    NS_LOG_FUNCTION  ( this << qkdv6Header );

    Buffer qkdv6HeaderBuffer;
    qkdv6HeaderBuffer.AddAtStart (qkdv6Header.GetSerializedSize());//20
    qkdv6Header.Serialize(qkdv6HeaderBuffer.Begin ());

    uint8_t *qkdv6Buffer = new uint8_t[qkdv6HeaderBuffer.GetSerializedSize() + 4];
    uint32_t serializeOutput = qkdv6HeaderBuffer.Serialize(qkdv6Buffer, qkdv6HeaderBuffer.GetSerializedSize() + 4);
    NS_LOG_FUNCTION(this << "QKD Header Serialize result: " << serializeOutput << qkdv6HeaderBuffer.GetSerializedSize() + 4);
    m_qkdv6HeaderSize = qkdv6HeaderBuffer.GetSerializedSize() + 4;

    //check if serialized process was sucessful
    NS_ASSERT(serializeOutput != 0);
 
    std::vector<uint8_t> qkdv6HeaderVector;
    //Add to headers vector which is going to be encrypted 
    for(uint16_t a=0; a<(qkdv6HeaderBuffer.GetSerializedSize() + 4); a++)
        qkdv6HeaderVector.push_back(qkdv6Buffer[a]);

    delete[] qkdv6Buffer;
    return qkdv6HeaderVector;
}

std::vector<uint8_t>
QKDv6Crypto::QKDv6DelimiterHeaderToVector(QKDv6DelimiterHeader& qkdv6DHeader)
{
    NS_LOG_FUNCTION  ( this << qkdv6DHeader );

    Buffer qkdv6DHeaderBuffer;
    qkdv6DHeaderBuffer.AddAtStart (qkdv6DHeader.GetSerializedSize());//20
    qkdv6DHeader.Serialize(qkdv6DHeaderBuffer.Begin ());

    uint8_t *qkdv6Buffer = new uint8_t[qkdv6DHeaderBuffer.GetSerializedSize() + 4];
    uint32_t serializeOutput = qkdv6DHeaderBuffer.Serialize(qkdv6Buffer, qkdv6DHeaderBuffer.GetSerializedSize() + 4);
    NS_LOG_FUNCTION(this << "QKD Delimiter Header Serialize result: " << serializeOutput << qkdv6DHeaderBuffer.GetSerializedSize() + 4);
    m_qkdv6DHeaderSize = qkdv6DHeaderBuffer.GetSerializedSize() + 4;

    NS_LOG_FUNCTION (this << "setting m_qkdv6DHeaderSize to" << m_qkdv6DHeaderSize);

    //check if serialized process was sucessful
    NS_ASSERT(serializeOutput != 0);
 
    std::vector<uint8_t> qkdv6DHeaderVector;
    //Add to headers vector which is going to be encrypted 
    for(uint16_t a=0; a<(qkdv6DHeaderBuffer.GetSerializedSize() + 4); a++){
        qkdv6DHeaderVector.push_back(qkdv6Buffer[a]);
    }

    delete[] qkdv6Buffer;
    return qkdv6DHeaderVector;
}

QKDv6Header
QKDv6Crypto::StringToQKDv6Header(std::string& input)
{
    NS_LOG_FUNCTION (this  
        << input.size()  
    );
 
    const uint8_t* qkdv6Buffer = reinterpret_cast<const uint8_t*>(input.c_str());  

    //Create QKDv6Buffer
    Buffer qkdv6HeaderBuffer;
    qkdv6HeaderBuffer.Deserialize( qkdv6Buffer, m_qkdv6HeaderSize ); 

    //Fetch QKDv6Header
    QKDv6Header qkdv6Header;
    qkdv6Header.Deserialize(qkdv6HeaderBuffer.Begin ());

    NS_LOG_FUNCTION (this << "QKD Header sucessfully Deserialized!" << qkdv6Header ); 
    //delete[] qkdv6Buffer;
...
QKDv6Crypto::StringToQKDv6DelimiterHeader(std::string& input)
{
    NS_LOG_FUNCTION (this  
        << input.size()  
    );
    ...
    //Create QKDv6Buffer
    Buffer qkdv6HeaderBuffer;
    qkdv6HeaderBuffer.Deserialize( qkdv6Buffer, m_qkdv6DHeaderSize ); 

    //Fetch QKDDelimiterHeader
    QKDv6DelimiterHeader qkdv6Header;
    qkdv6Header.Deserialize(qkdv6HeaderBuffer.Begin ());

    NS_LOG_FUNCTION (this << "QKD Delimiter Header sucessfully Deserialized!" << qkdv6Header ); 
    //delete qkdv6Buffer;
...
QKDv6CommandHeader
QKDv6Crypto::CreateQKDv6CommandHeader(Ptr<Packet> p)
{
    NS_LOG_FUNCTION(this << p);

    QKDv6CommandTag qkdv6CommandTag; 
    QKDv6CommandHeader qkdv6CommandHeader; 
    
    //Check whether this is routing message from routing protocol
    if(p->PeekPacketTag(qkdv6CommandTag)){
        /*
        std::cout << "\n......SENDING plain CreateQKDv6CommandHeader qkdv6CommandTag....." << p->GetUid() << "..." << p->GetSize() << ".....\n";
        qkdv6CommandTag.Print(std::cout);
        std::cout << "\n.............................\n"; 
        */
        if(qkdv6CommandTag.GetCommand () == 'R')
            NS_LOG_FUNCTION (this << "IT IS ROUTING HELLO PACKET!" <<  qkdv6CommandTag.GetCommand()  );
        else {
            NS_LOG_FUNCTION (this << "IT IS NORMAL PACKET!" << qkdv6CommandTag.GetCommand () );
        }

        NS_LOG_FUNCTION (this << "RoutingProtocolNumber:" << qkdv6CommandTag.GetRoutingProtocolNumber ());
        qkdv6CommandHeader.SetCommand ( qkdv6CommandTag.GetCommand() );//Routing hello message

    }else
        NS_LOG_FUNCTION (this << "PACKET WITHOUT QKDCOMMAND TAG RECEIVED!");
    
    return qkdv6CommandHeader;
}


bool 
QKDv6Crypto::CheckForResourcesToProcessThePacket(
    Ptr<Packet>             p, 
    uint32_t                TOSBand,
    ...
    {
        case QKDv6CRYPTO_OTP:
            keySize += p->GetSize() * 8;//in bits
            break;
            
        case QKDv6CRYPTO_AES:  
            keySize += CryptoPP::AES::MAX_KEYLENGTH;
            break;
    } 
    
    //KEY IS NEEDED ONLY FOR VMAC
    if(shouldAuthenticate == QKDv6CRYPTO_AUTH_VMAC){
        keySize += m_authenticationTagLengthInBits ;
    }

    uint32_t QKDv6bufferStatus = QKDv6buffer->FetchState(); 
    uint32_t Mcur = QKDv6buffer->GetMcurrent();

    NS_LOG_DEBUG ( this 
    << "\tQKDv6Buffer: \t" << QKDv6bufferStatus 
    << "\tTOSband: \t" << TOSBand
    << "\tKeySize: \t" << keySize 
    << "\n");
    
    NS_LOG_DEBUG ( this 
    << "\tQKDv6Buffer Material: \t" << Mcur  
    << "\tKeySize: \t" << keySize 
    << "\t Enough resources: \t" << (float) (Mcur > keySize)
    << "\n");

    //if buffer is empty then only top priority packet can be transmitted
    if(QKDv6bufferStatus == QKDv6Buffer::QKDSTATUS_EMPTY && TOSBand != 0){
...
QKDv6Crypto::ProcessOutgoingPacket (
    Ptr<Packet>             p, 
    Ptr<QKDv6Buffer>          QKDv6buffer,
    uint32_t                channelID
)
{
    ...
    bool QKDv6InternalTOSTagPresent = false;
    QKDv6InternalTOSTag qkdNextHopTag; 
    if(p->RemovePacketTag(qkdNextHopTag))
        QKDv6InternalTOSTagPresent = true; 
    ...
    /* 
        ...
        Consider following example: 
            ...
            |QKDv6Header|***Payload***||QKDv6Header|*************Payload|*************|
            ...
            small piece of QKD Header, and then we need to deal with half-QKDv6header. Therefore, we decide to convert everything into string 
            (payload and QKDv6Header), which is easeier for manipulation and storing. 
    ...
    */
    ...
    if(shouldEncrypt > 0){
        ...
        switch (shouldEncrypt)
        {
            case QKDv6CRYPTO_OTP:
                ...
                if(QKDv6buffer != 0) 
                    key = QKDv6buffer->ProcessOutgoingRequest ( plainText.size() * 8 ); //In bits
            ...
            case QKDv6CRYPTO_AES: 
                ...
                if(QKDv6buffer != 0) 
                    key = QKDv6buffer->ProcessOutgoingRequest ( CryptoPP::AES::MAX_KEYLENGTH ); //AES in bits
        ...
        qkdv6Header.SetEncryptionKeyId(key->GetUid()); 
        qkdv6Header.SetEncrypted (shouldEncrypt);
    ...
    qkdv6Header.SetMessageId( p->GetUid() );
    qkdv6Header.SetChannelId(channelID); 
    qkdv6Header.SetLength(m_qkdv6HeaderSize + cipherText.size());
    ...
    if (shouldEncrypt>0 || shouldAuthenticate>0){
        ...
        if(shouldAuthenticate == QKDv6CRYPTO_AUTH_VMAC){
            ...
            if(QKDv6buffer != 0) 
                key = QKDv6buffer->ProcessOutgoingRequest ( m_authenticationTagLengthInBits ); //In bits
        ...
        qkdv6Header.SetAuthenticationKeyId(key->GetUid()); 
        qkdv6Header.SetAuthTag(authTag); 
        qkdv6Header.SetAuthenticated (shouldAuthenticate);
        ...
        qkdv6Header.SetAuthenticated (0);
        qkdv6Header.SetEncrypted (0);
    }
    
    /*
     *  Now convert QKDv6Header to std::string and merge it with the rest of the packet
    */

    //QKD Header is now ready and it can be encoded into packet
    std::vector<uint8_t> qkdv6HeaderInVector = QKDv6HeaderToVector(qkdv6Header);

    //Convert whole packet into vector
    std::vector<uint8_t> packetContentInVector = StringToVector( cipherText );

    //Now add qkdv6header in the begining of packet vector
    qkdv6HeaderInVector.insert(qkdv6HeaderInVector.end(), packetContentInVector.begin(), packetContentInVector.end());

    //Finally convert everything to one string
    std::string finalPayload = VectorToString(qkdv6HeaderInVector);
    ...
    NS_LOG_FUNCTION(this 
        ...
        << "MessageID:" << qkdv6Header.GetMessageId()
        << "QKDv6HeaderLength:" << qkdv6Header.GetLength()
        << "ChannelID:" << channelID
    );
...
std::string
QKDv6Crypto::PacketToString (Ptr<Packet> p)
{ 
    ...
    std::cout << "\n ............................................  \n"; 
    */
    QKDv6CommandHeader qkdv6CommandHeader = CreateQKDv6CommandHeader(p);

    PacketMetadata::Item item; 
    PacketMetadata::ItemIterator metadataIterator = p->BeginItem();

    uint16_t counter = 0; 
    uint32_t firstNextHeader = 6; //IPv6 by default
    ...
    while (metadataIterator.HasNext()) {
        ...
        {             
            ...
            // NS_LOG_FUNCTION(this << uint32_t (ipv6Header->GetProtocol()) << qkdv6CommandHeader);

            Buffer ipHeaderBuffer;
            ipHeaderBuffer.AddAtStart (ipv6Header->GetSerializedSize());
             
            // NS_LOG_FUNCTION (this << "Ipv6Header Protocol value: " << (uint32_t) ipv6Header->GetProtocol() );
            ipv6Header->Serialize(ipHeaderBuffer.Begin ());
            /*
            std::cout << "\n....IPv4 HEADER....\n"; 
            ...
            */
            ...
            headerContentSize += ipv6Header->GetSerializedSize();
            NS_LOG_FUNCTION(this << "IPv4 Header size: " << ipv6Header->GetSerializedSize());
            delete ipv6Header; 
            ...
            //SET HEADER SIZE
            m_ipv6HeaderSize = ipHeaderBuffer.GetSerializedSize() + 4;
        ...
        ////////////////////////////////////////
        //  ICMPv6 Header Serialize
        ///////////////////////////////////////

        }else if(item.tid.GetName() == "ns3::Icmpv6Header") 
        { 
            Callback<ObjectBase *> constr = item.tid.GetConstructor();
            NS_ASSERT(!constr.IsNull());
            ...
            Icmpv6Header* icmpv6Header = dynamic_cast<Icmpv6Header*> (instance);
            NS_ASSERT(icmpv6Header != 0);
            icmpv6Header->Deserialize(item.current);
            /*      
            std::cout << "\n....icmpv6Header HEADER....\n"; 
            icmpv6Header->Print(std::cout);
            std::cout << "\n...................\n";
            */
            Buffer icmpv6HeaderBuffer;
            icmpv6HeaderBuffer.AddAtStart (icmpv6Header->GetSerializedSize());//8
            icmpv6Header->Serialize(icmpv6HeaderBuffer.Begin ());

            // finished, clear the header
            headerContentSize += icmpv6Header->GetSerializedSize(); 
            NS_LOG_FUNCTION(this << "icmpv6Header  size: " << icmpv6Header->GetSerializedSize());
            delete icmpv6Header; 

            uint8_t *icmpv4Buffer = new uint8_t[icmpv6HeaderBuffer.GetSerializedSize() + 4];
            uint32_t serializeOutput = icmpv6HeaderBuffer.Serialize(icmpv4Buffer, icmpv6HeaderBuffer.GetSerializedSize() + 4);
            NS_LOG_FUNCTION(this << "Icmpv6Header Header Serialize result: " << serializeOutput << icmpv6HeaderBuffer.GetSerializedSize() + 4);

            //SET HEADER SIZE
            m_icmpv6HeaderSize = icmpv6HeaderBuffer.GetSerializedSize() + 4;

            //check if serialized process was sucessful
            NS_ASSERT(serializeOutput != 0);
            
            //Add to headers vector which is going to be encrypted
            for(uint16_t a=0; a<(icmpv6HeaderBuffer.GetSerializedSize() + 4); a++)
                headerContentVector.push_back(icmpv4Buffer[a]);

            NS_LOG_FUNCTION ("PLAINTEXT.size after Icmpv6Header: " << headerContentVector.size() );
        ...
        }else if(item.tid.GetName() == "ns3::Icmpv6DestinationUnreachable") 
        { 
            Callback<ObjectBase *> constr = item.tid.GetConstructor();
            NS_ASSERT(!constr.IsNull());
            ...
            Icmpv6DestinationUnreachable* header = dynamic_cast<Icmpv6DestinationUnreachable*> (instance);
            NS_ASSERT(header != 0);
            ...
            */
            ...
            NS_LOG_FUNCTION(this << "Icmpv6DestinationUnreachable Header Serialize result: " << serializeOutput << headerBuffer.GetSerializedSize() + 4);

            //SET HEADER SIZE
            m_icmpv6DestinationUnreachableHeaderSize = headerBuffer.GetSerializedSize() + 4;
        ...
        { 
            ...
            Icmpv6Echo* header = dynamic_cast<Icmpv6Echo*> (instance);
            NS_ASSERT(header != 0);
            header->Deserialize(item.current);
            /*      
            std::cout << "\n....header HEADER....\n"; 
            ...
            */
            ...
            //SET HEADER SIZE
            m_icmpv6EchoHeaderSize = headerBuffer.GetSerializedSize() + 4;
        ...
        { 
            ...
            Icmpv6TimeExceeded* header = dynamic_cast<Icmpv6TimeExceeded*> (instance);
            NS_ASSERT(header != 0);
            header->Deserialize(item.current);
            /*      
            std::cout << "\n....header HEADER....\n"; 
            ...
            */
            ...
            //SET HEADER SIZE
            m_icmpv6TimeExceededHeaderSize = headerBuffer.GetSerializedSize() + 4;
        ...
        { 
            ...
            */
            ...
            qkdv6DHeader.SetDelimiterSize ( tcpHeaderBuffer.GetSerializedSize() + 4 ); 
    ...
    NS_LOG_FUNCTION (this << "qkdv6CommandHeader is to be added" << qkdv6CommandHeader);

    /*
    std::cout << "\n......SENDING plain qkdv6CommandHeader....." << p->GetUid() << "..." << p->GetSize() << ".....\n";
    qkdv6CommandHeader.Print(std::cout);
    std::cout << "\n.............................\n"; 
    */

    // SERIALIZE QKDCOMMAND HEADER
    NS_LOG_FUNCTION (this << "qkdv6CommandHeader.GetCommand" << qkdv6CommandHeader.GetCommand () );
    NS_LOG_FUNCTION (this << "qkdv6CommandHeader.GetRoutingProtocolNumber" << (uint32_t) qkdv6CommandHeader.GetProtocol () );

    //if it is routing routing protocol message
    
    /*
    QKDv6CommandHeader points to IPv4 which points to TCP/UDP by default. 
    However, when there is GSPRQ header siting between IPv4 and TCP/UDP, 
    QKDv6CommandHeader has different values:

        value 4 : Header order is following and it is used by default: 
            QKDv6Header, QKDv6CommandHeader, IPv4, TCP/UDP

        DSDVQ_HEADER_PROTOCOL_NUMBER : Header order is following: 
            QKDv6Header, QKDv6CommandHeader, IPv4, DSDVQ

        DSDV_HEADER_PROTOCOL_NUMBER : Header order is following: 
            QKDv6Header, QKDv6CommandHeader, IPv4, DSDV
    */

    QKDv6CommandTag qkdv6CommandTag;        

    if( p->PeekPacketTag(qkdv6CommandTag) && 
        (qkdv6CommandTag.GetCommand () == 'R' ||
            ( qkdv6CommandTag.GetRoutingProtocolNumber () == DSDV_PACKET_HEADER_PROTOCOL_NUMBER // DSDV
            || qkdv6CommandTag.GetRoutingProtocolNumber () == DSDVQ_PACKET_HEADER_PROTOCOL_NUMBER // DSDVQ
            )
        )
    ){
        NS_LOG_FUNCTION (this << "Need to fix QKDv6CommandHeader Protocol field to : " << qkdv6CommandTag.GetRoutingProtocolNumber ());
        
        /*
        std::cout << "\n......SENDING plain qkdv6CommandTag....." << p->GetUid() << "..." << p->GetSize() << ".....\n";
        qkdv6CommandTag.Print(std::cout);
        std::cout << "\n.............................\n"; 
        */

        qkdv6CommandHeader.SetProtocol(  static_cast<uint32_t> (qkdv6CommandTag.GetRoutingProtocolNumber ()) ); 
        p->RemovePacketTag(qkdv6CommandTag); 
        
        if(qkdv6CommandHeader.GetProtocol () == DSDV_PACKET_HEADER_PROTOCOL_NUMBER || qkdv6CommandHeader.GetProtocol () == DSDVQ_PACKET_HEADER_PROTOCOL_NUMBER)
            NS_ASSERT (qkdv6CommandHeader.GetCommand () == 'R' );
        
    }else{
        NS_LOG_FUNCTION (this << "PACKET WITHOUT QKDCOMMAND TAG RECEIVED!");
        qkdv6CommandHeader.SetProtocol( 4 ); //IPv4 by default
    }
    NS_LOG_FUNCTION (this << "qkdv6CommandHeader protocol:" << qkdv6CommandHeader.GetProtocol());

    Buffer qkdv6CommandHeaderBuffer;
    qkdv6CommandHeaderBuffer.AddAtStart (qkdv6CommandHeader.GetSerializedSize());
    qkdv6CommandHeader.Serialize(qkdv6CommandHeaderBuffer.Begin ());
 
    uint8_t *qkdcBuffer = new uint8_t[qkdv6CommandHeaderBuffer.GetSerializedSize() + 4];
    uint32_t serializeOutput = qkdv6CommandHeaderBuffer.Serialize(qkdcBuffer, qkdv6CommandHeaderBuffer.GetSerializedSize() + 4);
 
    //check if serialized process was sucessful
    NS_ASSERT(serializeOutput != 0);
    NS_LOG_FUNCTION(this << "QKDCommand Header Serialize result: " << serializeOutput << qkdv6CommandHeaderBuffer.GetSerializedSize() + 4);
    NS_LOG_FUNCTION(this << "QKDCommand Header size: " << qkdv6CommandHeaderBuffer.GetSerializedSize() );
    headerContentSize += qkdv6CommandHeaderBuffer.GetSerializedSize();
 
    //Add to headers vector which is going to be encrypted 
    for(uint16_t a=0; a<(qkdv6CommandHeaderBuffer.GetSerializedSize() + 4); a++)
...
std::vector<Ptr<Packet> > 
QKDv6Crypto::CheckForFragmentation(Ptr<Packet> p, Ptr<QKDv6Buffer> QKDv6Buffer)
{   
    uint32_t channelID = 0;

    if(QKDv6Buffer != 0) 
        channelID = QKDv6Buffer->m_bufferID;
    ...
    std::string qkdv6HeaderText = cipherTextTotal.substr (0, m_qkdv6HeaderSize );
    NS_LOG_FUNCTION (this << "Croping QKD HEADER! of size " << qkdv6HeaderText.size() );

    QKDv6Header qkdv6Header = StringToQKDv6Header(qkdv6HeaderText);

    //Ok ommit first 48 bytes and create a new QKD packet with that content

    cipherTextTotal = cipherTextTotal.substr (m_qkdv6HeaderSize, cipherTextTotal.size() - m_qkdv6HeaderSize );
    Ptr<Packet> packetWithQKDv6Header = Create<Packet> ((uint8_t*) cipherTextTotal.c_str(), cipherTextTotal.size());
    packetWithQKDv6Header->AddHeader(qkdv6Header);

    NS_LOG_INFO (this << "packetWithQKDv6Header: " << packetWithQKDv6Header);
    ...
    uint32_t pointer = 0; 
    bool newQkdv6HeaderNeeded = false;
    ...
    while(pointer < cipherTextTotal.size()){
        ...
        if(newQkdv6HeaderNeeded && cipherTextTotal.size() - pointer <= m_qkdv6HeaderSize){
        ...
        NS_LOG_FUNCTION (this << "Do we need new QKD HEADER:" << newQkdv6HeaderNeeded);
        QKDv6Header qkdv6HeaderTemp;
        if(newQkdv6HeaderNeeded){

            //detech qkd header first
            NS_LOG_FUNCTION( this << "Detach QKD header first from pointer:" << pointer );
            std::string tempQKDv6HeaderText = cipherTextTotal.substr (pointer, m_qkdv6HeaderSize);

            qkdv6HeaderTemp = StringToQKDv6Header(tempQKDv6HeaderText);
            pointer += m_qkdv6HeaderSize;
            NS_LOG_FUNCTION(this << "Pointer:" << pointer);

        }else{
            qkdv6HeaderTemp = qkdv6Header;
            NS_LOG_FUNCTION(this << "qkdv6HeaderTemp:" << qkdv6HeaderTemp);
        }
        ...
        */
        if( cipherTextLength - pointer >= qkdv6HeaderTemp.GetLength() - m_qkdv6HeaderSize ){

            NS_LOG_FUNCTION (this << "We have enough material to decrypt the packet!" );

            //we have enough information to decrypt the packet
            std::string tempPacketPayload = cipherTextTotal.substr (pointer, qkdv6HeaderTemp.GetLength() - m_qkdv6HeaderSize); 
            ...
            pointer += qkdv6HeaderTemp.GetLength() - m_qkdv6HeaderSize;
            NS_LOG_FUNCTION(this << "Pointer:" << pointer);

            tempPacket->AddHeader(qkdv6HeaderTemp);
            packetOutput.push_back(tempPacket);
            newQkdv6HeaderNeeded = true;
            ...
            if(pointer == 0){
                ...
                uint32_t cacheFrom = pointer - m_qkdv6HeaderSize;
...
std::vector<Ptr<Packet> >
QKDv6Crypto::ProcessIncomingPacket (
    Ptr<Packet>             p, 
    Ptr<QKDv6Buffer>          QKDv6Buffer,
    uint32_t                channelID
)
{
    ...
    {  
        packetOutput[i] = Decrypt(packetOutput[i], QKDv6Buffer);
...
Ptr<Packet>
QKDv6Crypto::Decrypt (Ptr<Packet> p, Ptr<QKDv6Buffer> QKDv6buffer) 
{  
    NS_LOG_FUNCTION  ( this << p->GetUid() << p->GetSize() );
    ...
    */
    ...
    //Before decryption we need to remove the QKDv6header
    QKDv6Header qkdv6Header;

    //If no QKDv6header is found, then just return the packet since the packet probabily is not encrpyted!
    p->PeekHeader (qkdv6Header);
    if(!qkdv6Header.IsValid () ){
        NS_LOG_FUNCTION(this << "UNKNOWN packet received!");
        return p;
    }

    NS_LOG_FUNCTION(this << qkdv6Header);
    if (qkdv6Header.GetAuthenticated() > 1) {
 
        Ptr<QKDv6Key> key;
        //KEY IS NEEDED ONLY FOR VMAC
        if(qkdv6Header.GetAuthenticated() == QKDv6CRYPTO_AUTH_VMAC){
            
            if(QKDv6buffer != 0) 
                key = QKDv6buffer->ProcessIncomingRequest ( qkdv6Header.GetAuthenticationKeyId(), m_authenticationTagLengthInBits ); //in bits
        ...
        Ptr<Packet> pTemp = CheckAuthentication(p, key, qkdv6Header.GetAuthenticated() );
        if(pTemp == 0){
            NS_LOG_FUNCTION(this << "Authentication tag is not valid!");
            return 0;return p;   
        }
    }
    p->RemoveHeader (qkdv6Header); 
    ...
    if (qkdv6Header.GetEncrypted() > 0){
        NS_ASSERT (qkdv6Header.GetEncrypted() > 0);

        Ptr<QKDv6Key> key;
        switch (qkdv6Header.GetEncrypted())
        {
            case QKDv6CRYPTO_OTP: 

                if(QKDv6buffer != 0) 
                    key = QKDv6buffer->ProcessIncomingRequest ( qkdv6Header.GetEncryptionKeyId(), cipherText.size() * 8 );  //in bits
            ...
            case QKDv6CRYPTO_AES: 
                ...
                if(QKDv6buffer != 0) 
                    key = QKDv6buffer->ProcessIncomingRequest ( qkdv6Header.GetEncryptionKeyId(), CryptoPP::AES::MAX_KEYLENGTH ); // in bits
    ...
    // DECOMPRESS PACKET 
    if(qkdv6Header.GetZipped () == 1){
        plainText = StringDecompressDecode (plainText); 
    }

    if ( plainText.size() <= m_ipv6HeaderSize){
    ...
    std::string ipHeaderPlainText = plainText.substr (0, m_ipv6HeaderSize);
    plainText = plainText.substr(m_ipv6HeaderSize, plainText.size() - m_ipv6HeaderSize);
    ...
    Buffer ipHeaderBuffer;
    ipHeaderBuffer.Deserialize(ipBuffer, m_ipv6HeaderSize); 
    //delete[] ipBuffer;

    Ipv6Header ipv6Header;
    ipv6Header.Deserialize(ipHeaderBuffer.Begin ());
    ...
    //First remove QKDv6CommandHeader 
    ...
    NS_LOG_FUNCTION ("DECRYPTED PLAINTEXT.size after IPv4 and qkdv6CommandHeader: " << plainText.size() );
    ...
    //Fetch QKDCHeader
    QKDv6CommandHeader qkdv6CommandHeader;
    qkdv6CommandHeader.Deserialize(qkdcHeaderBuffer.Begin ());

    NS_LOG_FUNCTION (this << "qkdv6CommandHeader Deserialized" << qkdv6CommandHeader);
    ...
    */
    ...
    QKDv6CommandHeader has different values:
        ...
        DSDV_HEADER_PROTOCOL_NUMBER - Header order is following: 
            QKDv6Header, QKDv6CommandHeader, IPv4, DSDV
    */        
    NS_LOG_FUNCTION (this << "qkdv6CommandHeader Protocol Value" << qkdv6CommandHeader.GetProtocol() );

    bool allowIPv4ToPushHeaderInProtocolChain = true;
    if(qkdv6CommandHeader.GetProtocol () == 4){ 
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == DSDVQ_PACKET_HEADER_PROTOCOL_NUMBER){//147
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == DSDV_PACKET_HEADER_PROTOCOL_NUMBER){//147
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODV_TYPE_HEADER_PROTOCOL_NUMBER){//41
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODV_RREQ_HEADER_PROTOCOL_NUMBER){//42
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODV_RREP_HEADER_PROTOCOL_NUMBER){//43
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODV_RREP_ACK_HEADER_PROTOCOL_NUMBER){//44
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODV_RERR_HEADER_PROTOCOL_NUMBER){//45
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODVQ_TYPE_HEADER_PROTOCOL_NUMBER){//41
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODVQ_RREQ_HEADER_PROTOCOL_NUMBER){//42
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODVQ_RREP_HEADER_PROTOCOL_NUMBER){//43
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODVQ_RREP_ACK_HEADER_PROTOCOL_NUMBER){//44
        ...
        }else if(qkdv6CommandHeader.GetProtocol() == AODVQ_RERR_HEADER_PROTOCOL_NUMBER){//45
        ...
        // }else if(qkdv6CommandHeader.GetProtocol() == 4 && ipv4Header.GetProtocol() == 1){
        }else if(qkdv6CommandHeader.GetProtocol()){
    ...
    ns3::Icmpv6Echo Icmpv6EchoHeader;
    ns3::Icmpv6DestinationUnreachable Icmpv6DestinationUnreachableHeader;
    ns3::Icmpv6TimeExceeded Icmpv6TimeExceededHeader;
    ...
    {  
        ...
        //////////////////////////////////////
            ...
            std::string icmpv6HeaderText = plainText.substr (counter,counter + m_icmpv6HeaderSize);
            const uint8_t* icmpv4Buffer = reinterpret_cast<const uint8_t*>(icmpv6HeaderText.c_str());

            Buffer icmpv6HeaderBuffer;
            icmpv6HeaderBuffer.Deserialize(icmpv4Buffer, m_icmpv6HeaderSize);  

            Icmpv6Header.Deserialize(icmpv6HeaderBuffer.Begin ()); 
 
            counter += m_icmpv6HeaderSize;

            NS_LOG_FUNCTION(this << "ICMPv4 Header decrypted");
            NS_LOG_FUNCTION(this << "ICMPv4 Header;Type value:" << (uint32_t) Icmpv6Header.GetType() );
            /*
            std::cout << "\n";
            ...
            */
            ...
            protocols.push_back(static_cast<uint32_t> (Icmpv6Header.GetType()));
            //delete[] icmpv4Buffer;
 
        }else if(protocols[i] == Icmpv6Header::ICMPV6_ECHO_REQUEST || protocols[i] == Icmpv6Header::ICMPV6_ECHO_REPLY){// || ECHO_REPLY

            NS_LOG_FUNCTION(this << "ICMPv4 ECHO detected!" << counter);
 
            std::string icmpv6HeaderText = plainText.substr (counter,counter + m_icmpv6EchoHeaderSize);
            const uint8_t* icmpv4Buffer = reinterpret_cast<const uint8_t*>(icmpv6HeaderText.c_str());

            Buffer icmpv6HeaderBuffer;
            icmpv6HeaderBuffer.Deserialize(icmpv4Buffer, m_icmpv6EchoHeaderSize);  

            Icmpv6EchoHeader.Deserialize(icmpv6HeaderBuffer.Begin ()); 
 
            counter += m_icmpv6EchoHeaderSize;

            /*
            std::cout << "\n";
            Icmpv6EchoHeader.Print(std::cout);                
            std::cout << "\n";
            */
        ...
        }else if(protocols[i] == Icmpv6Header::ICMPV6_ERROR_DESTINATION_UNREACHABLE){

            NS_LOG_FUNCTION(this << "ICMPv4 DEST_UNREACH detected!" << counter);
 
            std::string icmpv6HeaderText = plainText.substr (counter,counter + m_icmpv6DestinationUnreachableHeaderSize);
            const uint8_t* icmpv4Buffer = reinterpret_cast<const uint8_t*>(icmpv6HeaderText.c_str());

            Buffer icmpv6HeaderBuffer;
            icmpv6HeaderBuffer.Deserialize(icmpv4Buffer, m_icmpv6DestinationUnreachableHeaderSize);  

            Icmpv6DestinationUnreachableHeader.Deserialize(icmpv6HeaderBuffer.Begin ()); 
 
            counter += m_icmpv6DestinationUnreachableHeaderSize;

            NS_LOG_FUNCTION(this << "ICMPv4 DEST_UNREACH decrypted"); 
            /*
            std::cout << "\n";
            Icmpv6DestinationUnreachableHeader.Print(std::cout);                
            std::cout << "\n";
            */
            //delete[] icmpv4Buffer;

        }else if(protocols[i] == Icmpv6Header::ICMPV6_ERROR_TIME_EXCEEDED){

            NS_LOG_FUNCTION(this << "ICMPv4 TIME_EXCEEDED detected!" << counter);
 
            std::string icmpv6HeaderText = plainText.substr (counter,counter + m_icmpv6TimeExceededHeaderSize);
            const uint8_t* icmpv4Buffer = reinterpret_cast<const uint8_t*>(icmpv6HeaderText.c_str());

            Buffer icmpv6HeaderBuffer;
            icmpv6HeaderBuffer.Deserialize(icmpv4Buffer, m_icmpv6TimeExceededHeaderSize);  

            Icmpv6TimeExceededHeader.Deserialize(icmpv6HeaderBuffer.Begin ()); 
 
            counter += m_icmpv6TimeExceededHeaderSize;

            NS_LOG_FUNCTION(this << "ICMPv4 TIME_EXCEEDED decrypted"); 
            /*
            std::cout << "\n";
            Icmpv6TimeExceededHeader.Print(std::cout);                
            std::cout << "\n";
            */
            //delete[] icmpv4Buffer;
        ...
        //////////////////////////////////////
            ...
            NS_LOG_FUNCTION (this << "TCP size should be " << qkdv6DHeader.GetDelimiterSize());
 
            std::string tcpHeaderText = plainText.substr(counter, qkdv6DHeader.GetDelimiterSize());  
    ...
    while(i > 0){   
        ...
        switch (protocols[i-1]){
            ...
            case 0:
                ...
                NS_LOG_FUNCTION(this << "Icmpv6Header added to packet!");
            break;
            case Icmpv6Header::ICMPV6_ECHO_REQUEST:
                packet->AddHeader (Icmpv6EchoHeader);
                NS_LOG_FUNCTION(this << "Icmpv6Header::ICMPV6_ECHO header added to packet!");
            break;
            case Icmpv6Header::ICMPV6_ECHO_REPLY:
                packet->AddHeader (Icmpv6EchoHeader);
                NS_LOG_FUNCTION(this << "Icmpv6Header::ICMPV6_ECHO_REPLY header added to packet!");
            break;
            case Icmpv6Header::ICMPV6_ERROR_DESTINATION_UNREACHABLE:
                packet->AddHeader (Icmpv6DestinationUnreachableHeader);
                NS_LOG_FUNCTION(this << "Icmpv6DestinationUnreachable Header added to packet!");
            break;
            case Icmpv6Header::ICMPV6_ERROR_TIME_EXCEEDED:
                packet->AddHeader (Icmpv6TimeExceededHeader);
                NS_LOG_FUNCTION(this << "Icmpv6TimeExceeded Header added to packet!");
            break;
    ...
    QKDv6InternalTag internalTag; 
    internalTag.SetAuthenticateValue ( qkdv6Header.GetAuthenticated() ); 
    internalTag.SetEncryptValue ( qkdv6Header.GetEncrypted() );
    packet->AddPacketTag(internalTag); 
      
    /*
    packet->AddHeader(qkdv6CommandHeader);
    std::cout << "\n ..................RECEIVED plain....." << packet->GetUid() << "..." <<  packet->GetSize() << ".......  \n";
    packet->Print(std::cout);                
    std::cout << "\n ............................................  \n";
    packet->RemoveHeader(qkdv6CommandHeader);
    */
...
std::string
QKDv6Crypto::OTP (const std::string& data, Ptr<QKDv6Key> key)
{
...
std::string 
QKDv6Crypto::AESEncrypt (const std::string& data, Ptr<QKDv6Key> key)
{
    NS_LOG_FUNCTION  (this << data.size());  
    if(!m_encryptionEnabled) return data;
...
std::string 
QKDv6Crypto::AESDecrypt (const std::string& data, Ptr<QKDv6Key> key)
{ 
    NS_LOG_FUNCTION  (this << data.size()); 
    if(!m_encryptionEnabled) return data;
...
std::string 
QKDv6Crypto::HexEncode(const std::string& data)
{
    NS_LOG_FUNCTION  (this << data.size()); 
    if(!m_encryptionEnabled) return data;
...
std::string 
QKDv6Crypto::HexDecode(const std::string& data)
{
    NS_LOG_FUNCTION  (this << data.size()); 
    if(!m_encryptionEnabled) return data;
...
std::string
QKDv6Crypto::Authenticate (std::string& inputString, Ptr<QKDv6Key> key, uint8_t authenticationType)
{ 
    NS_LOG_FUNCTION (this << inputString.length()); 

    switch (authenticationType)
    {
        case QKDv6CRYPTO_AUTH_VMAC:  
            return VMAC(inputString, key); 
            break;
        case QKDv6CRYPTO_AUTH_MD5: 
            return MD5(inputString);
            break;
        case QKDv6CRYPTO_AUTH_SHA1: 
...
Ptr<Packet>
QKDv6Crypto::CheckAuthentication(Ptr<Packet> p, Ptr<QKDv6Key> key, uint8_t authenticationType)
{   
    NS_LOG_FUNCTION  (this << p->GetUid() );   

    QKDv6Header qkdv6Header;
    p->PeekHeader(qkdv6Header);
    //If no QKDv6header is found, then just return the packet since the packet probabily is not authenticated! 
    if(!qkdv6Header.IsValid () || qkdv6Header.GetAuthenticated() <= 0){
        NS_LOG_FUNCTION(this << "Unauthenticated packet received!");
        return p;
    }

    Ptr<Packet> packet = p->Copy ();
    packet->RemoveHeader(qkdv6Header);
    ...
    if(hashTag != qkdv6Header.GetAuthTag()){

        NS_LOG_FUNCTION (this << "1:" << hashTag << hashTag.length() );
        NS_LOG_FUNCTION (this << "2:" << qkdv6Header.GetAuthTag() << qkdv6Header.GetAuthTag().length()  );
...
std::string
QKDv6Crypto::VMAC (std::string& inputString, Ptr<QKDv6Key> key)
{ 
...
std::string 
QKDv6Crypto::MD5(std::string& inputString)
{   
    NS_LOG_FUNCTION (this << inputString.length() );   
    if(!m_encryptionEnabled) 
        return std::string( m_authenticationTagLengthInBits, '0'); 
...
std::string 
QKDv6Crypto::SHA1(std::string& inputString)
{   
    NS_LOG_FUNCTION (this << inputString.length() );   
    if(!m_encryptionEnabled) 
        return std::string( m_authenticationTagLengthInBits, '0'); 
...
std::string 
QKDv6Crypto::base64_encode(std::string& s) 
{
...
std::string 
QKDv6Crypto::base64_decode(std::string const& encoded_string) 
{
...
std::string 
QKDv6Crypto::StringCompressEncode(const std::string &data)
{
...
std::string 
QKDv6Crypto::StringDecompressDecode(const std::string &data)
{
    NS_LOG_FUNCTION (this << "INPUT:" << data.length());  
    if(!m_compressionEnabled) return data;
