...
NS_LOG_COMPONENT_DEFINE ("VirtualTcpL4Protocol");

NS_OBJECT_ENSURE_REGISTERED (VirtualTcpL4Protocol);

//VirtualTcpL4Protocol stuff----------------------------------------------------------
...
const uint8_t VirtualTcpL4Protocol::PROT_NUMBER = 6;

TypeId
VirtualTcpL4Protocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::VirtualTcpL4Protocol")
    .SetParent<IpL4Protocol> ()
    .SetGroupName ("Internet")
    .AddConstructor<VirtualTcpL4Protocol> ()
    .AddAttribute ("RttEstimatorType",
                   "Type of RttEstimator objects.",
                   TypeIdValue (RttMeanDeviation::GetTypeId ()),
                   MakeTypeIdAccessor (&VirtualTcpL4Protocol::m_rttTypeId),
                   MakeTypeIdChecker ())
    .AddAttribute ("SocketType",
                   "Socket type of TCP objects.",
                   TypeIdValue (TcpNewReno::GetTypeId ()),
                   MakeTypeIdAccessor (&VirtualTcpL4Protocol::m_congestionTypeId),
                   MakeTypeIdChecker ())
    .AddAttribute ("RecoveryType",
                   "Recovery type of TCP objects.",
                   TypeIdValue (TcpClassicRecovery::GetTypeId ()),
                   MakeTypeIdAccessor (&VirtualTcpL4Protocol::m_recoveryTypeId),
                   MakeTypeIdChecker ())
    .AddAttribute ("SocketList", "The list of sockets associated to this protocol.",
                   ObjectVectorValue (),
                   MakeObjectVectorAccessor (&VirtualTcpL4Protocol::m_sockets),
                   MakeObjectVectorChecker<VirtualTcpSocketBase> ())
  ;
  return tid;
}

VirtualTcpL4Protocol::VirtualTcpL4Protocol ()
  : m_endPoints (new Ipv4EndPointDemux ()), m_endPoints6 (new Ipv6EndPointDemux ())
{
  NS_LOG_FUNCTION_NOARGS ();
  NS_LOG_LOGIC ("Made a VirtualTcpL4Protocol " << this);
}
...
void
VirtualTcpL4Protocol::SetNode (Ptr<Node> node)
{
  NS_LOG_FUNCTION (this);
  m_node = node;
}
...
void
VirtualTcpL4Protocol::NotifyNewAggregate ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_node == 0)
    {
      if ((node != 0) && (ipv4 != 0 || ipv6 != 0))
        {
          this->SetNode (node);
          Ptr<VirtualTcpSocketFactoryImpl> tcpFactory = CreateObject<VirtualTcpSocketFactoryImpl> ();
...
int
VirtualTcpL4Protocol::GetProtocolNumber (void) const
{
  return PROT_NUMBER;
}

void
VirtualTcpL4Protocol::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_sockets.clear ();
...
Ptr<Socket>
VirtualTcpL4Protocol::CreateSocket (TypeId congestionTypeId)
{
  return CreateSocket (congestionTypeId, m_recoveryTypeId);
}

Ptr<Socket>
VirtualTcpL4Protocol::CreateSocket (TypeId congestionTypeId, TypeId recoveryTypeId)
{
  ...
  Ptr<VirtualTcpSocketBase> socket = CreateObject<VirtualTcpSocketBase> ();
...
Ptr<Socket>
VirtualTcpL4Protocol::CreateSocket (void)
{
  return CreateSocket (m_congestionTypeId, m_recoveryTypeId);
}

Ipv4EndPoint *
VirtualTcpL4Protocol::Allocate (void)
{
  NS_LOG_FUNCTION (this);
...
Ipv4EndPoint *
VirtualTcpL4Protocol::Allocate (Ipv4Address address)
{
...
Ipv4EndPoint *
VirtualTcpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice, uint16_t port)
{
...
Ipv4EndPoint *
VirtualTcpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice, Ipv4Address address, uint16_t port)
{
...
Ipv4EndPoint *
VirtualTcpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice,
...
void
VirtualTcpL4Protocol::DeAllocate (Ipv4EndPoint *endPoint)
{
...
Ipv6EndPoint *
VirtualTcpL4Protocol::Allocate6 (void)
{
  NS_LOG_FUNCTION (this);
...
Ipv6EndPoint *
VirtualTcpL4Protocol::Allocate6 (Ipv6Address address)
{
  NS_LOG_FUNCTION (this << address);
...
Ipv6EndPoint *
VirtualTcpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice, uint16_t port)
{
  NS_LOG_FUNCTION (this << boundNetDevice << port);
...
Ipv6EndPoint *
VirtualTcpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice, Ipv6Address address, uint16_t port)
{
  NS_LOG_FUNCTION (this << boundNetDevice << address << port);
...
Ipv6EndPoint *
VirtualTcpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice,
...
void
VirtualTcpL4Protocol::DeAllocate (Ipv6EndPoint *endPoint)
{
  NS_LOG_FUNCTION (this << endPoint);
...
void
VirtualTcpL4Protocol::ReceiveIcmp (Ipv4Address icmpSource, uint8_t icmpTtl,
...
void
VirtualTcpL4Protocol::ReceiveIcmp (Ipv6Address icmpSource, uint8_t icmpTtl,
                            uint8_t icmpType, uint8_t icmpCode, uint32_t icmpInfo,
...
VirtualTcpL4Protocol::PacketReceived (Ptr<Packet> packet, TcpHeader &incomingTcpHeader,
                               const Address &source, const Address &destination)
{
  ...
  NS_LOG_LOGIC ("VirtualTcpL4Protocol " << this
...
void
VirtualTcpL4Protocol::NoEndPointsFound (const TcpHeader &incomingHeader,
...
enum IpL4Protocol::RxStatus
VirtualTcpL4Protocol::Receive (Ptr<Packet> packet,
                        ...
                        Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if (endPoints.empty ())
    {
      ...
      NS_LOG_LOGIC ("VirtualTcpL4Protocol " << this << " received a packet but"
  ...
  NS_LOG_LOGIC ("VirtualTcpL4Protocol " << this << " received a packet and"
...
void
VirtualTcpL4Protocol::SendPacketV4 (Ptr<Packet> packet, const TcpHeader &outgoing,
                             ...
                             Ptr<NetDevice> oif) const
{
  ...
  Ptr<Ipv4> ipv4 =
    m_node->GetObject<VirtualIpv4L3Protocol> ();
  if (ipv4 != 0)
    {
      Ipv4Header header;
...
void
VirtualTcpL4Protocol::SendPacketV6 (Ptr<Packet> packet, const TcpHeader &outgoing,
...
void
VirtualTcpL4Protocol::SendPacket (Ptr<Packet> pkt, const TcpHeader &outgoing,
...
void
VirtualTcpL4Protocol::AddSocket (Ptr<VirtualTcpSocketBase> socket)
{
  NS_LOG_FUNCTION (this << socket);
  std::vector<Ptr<VirtualTcpSocketBase> >::iterator it = m_sockets.begin ();
...
bool
VirtualTcpL4Protocol::RemoveSocket (Ptr<VirtualTcpSocketBase> socket)
{
  NS_LOG_FUNCTION (this << socket);
  std::vector<Ptr<VirtualTcpSocketBase> >::iterator it = m_sockets.begin ();
...
void
VirtualTcpL4Protocol::SetDownTarget (IpL4Protocol::DownTargetCallback callback)
{
  m_downTarget = callback;
}

IpL4Protocol::DownTargetCallback
VirtualTcpL4Protocol::GetDownTarget (void) const
{
  return m_downTarget;
}

void
VirtualTcpL4Protocol::SetDownTarget6 (IpL4Protocol::DownTargetCallback6 callback)
{
  m_downTarget6 = callback;
}

IpL4Protocol::DownTargetCallback6
VirtualTcpL4Protocol::GetDownTarget6 (void) const
{
