...
NS_LOG_COMPONENT_DEFINE ("VirtualUdpL4Protocol");

NS_OBJECT_ENSURE_REGISTERED (VirtualUdpL4Protocol);

/* see http://www.iana.org/assignments/protocol-numbers */
const uint8_t VirtualUdpL4Protocol::PROT_NUMBER = 17;

TypeId 
VirtualUdpL4Protocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::VirtualUdpL4Protocol")
    .SetParent<IpL4Protocol> ()
    .SetGroupName ("Internet")
    .AddConstructor<VirtualUdpL4Protocol> ()
    .AddAttribute ("SocketList", "The list of sockets associated to this protocol.",
                   ObjectVectorValue (),
                   MakeObjectVectorAccessor (&VirtualUdpL4Protocol::m_sockets),
                   MakeObjectVectorChecker<VirtualUdpSocketImpl> ())
  ;
  return tid;
}

VirtualUdpL4Protocol::VirtualUdpL4Protocol ()
...
void 
VirtualUdpL4Protocol::SetNode (Ptr<Node> node)
{
  m_node = node;
}
 ...
 */
void
VirtualUdpL4Protocol::NotifyNewAggregate ()
{
  ...
  if (m_node == 0)
    {
      if ((node != 0) && (ipv4 != 0 || ipv6 != 0))
        {
          this->SetNode (node);
          Ptr<VirtualUdpSocketFactoryImpl> udpFactory = CreateObject<VirtualUdpSocketFactoryImpl> ();
...
int 
VirtualUdpL4Protocol::GetProtocolNumber (void) const
{
  return PROT_NUMBER;
}
...
void
VirtualUdpL4Protocol::DoDispose (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  for (std::vector<Ptr<VirtualUdpSocketImpl> >::iterator i = m_sockets.begin (); i != m_sockets.end (); i++)
    {
      *i = 0;
    }
  m_sockets.clear ();
...
Ptr<Socket>
VirtualUdpL4Protocol::CreateSocket (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  Ptr<VirtualUdpSocketImpl> socket = CreateObject<VirtualUdpSocketImpl> ();
...
Ipv4EndPoint *
VirtualUdpL4Protocol::Allocate (void)
{
  NS_LOG_FUNCTION (this);
...
Ipv4EndPoint *
VirtualUdpL4Protocol::Allocate (Ipv4Address address)
{
...
Ipv4EndPoint *
VirtualUdpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice, uint16_t port)
{
...
Ipv4EndPoint *
VirtualUdpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice, Ipv4Address address, uint16_t port)
{
  NS_LOG_FUNCTION (this << boundNetDevice << address << port);
  return m_endPoints->Allocate (boundNetDevice, address, port);
}
Ipv4EndPoint *
VirtualUdpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice,
...
void 
VirtualUdpL4Protocol::DeAllocate (Ipv4EndPoint *endPoint)
{
...
Ipv6EndPoint *
VirtualUdpL4Protocol::Allocate6 (void)
{
  NS_LOG_FUNCTION (this);
...
Ipv6EndPoint *
VirtualUdpL4Protocol::Allocate6 (Ipv6Address address)
{
  NS_LOG_FUNCTION (this << address);
...
Ipv6EndPoint *
VirtualUdpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice, uint16_t port)
{
  NS_LOG_FUNCTION (this << boundNetDevice << port);
...
Ipv6EndPoint *
VirtualUdpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice, Ipv6Address address, uint16_t port)
{
  NS_LOG_FUNCTION (this << boundNetDevice << address << port);
  return m_endPoints6->Allocate (boundNetDevice, address, port);
}
Ipv6EndPoint *
VirtualUdpL4Protocol::Allocate6 (Ptr<NetDevice> boundNetDevice,
...
void 
VirtualUdpL4Protocol::DeAllocate (Ipv6EndPoint *endPoint)
{
  NS_LOG_FUNCTION (this << endPoint);
...
void 
VirtualUdpL4Protocol::ReceiveIcmp (Ipv4Address icmpSource, uint8_t icmpTtl,
...
void 
VirtualUdpL4Protocol::ReceiveIcmp (Ipv6Address icmpSource, uint8_t icmpTtl,
                            uint8_t icmpType, uint8_t icmpCode, uint32_t icmpInfo,
...
VirtualUdpL4Protocol::Receive (Ptr<Packet> packet,
                        ...
                        Ptr<Ipv4Interface> interface)
{
  ...
  // 
  //   VirtualUdpL4Protocol::Receive (Ptr<Packet> packet, Ipv6Address &src, Ipv6Address &dst, ...)
...
void
VirtualUdpL4Protocol::Send (Ptr<Packet> packet, 
...
void
VirtualUdpL4Protocol::SetDownTarget (IpL4Protocol::DownTargetCallback callback)
{
  NS_LOG_FUNCTION (this);
...
VirtualUdpL4Protocol::GetDownTarget (void) const
{
  return m_downTarget;
}

void
VirtualUdpL4Protocol::SetDownTarget6 (IpL4Protocol::DownTargetCallback6 callback)
{
  NS_LOG_FUNCTION (this);
...
VirtualUdpL4Protocol::GetDownTarget6 (void) const
{
