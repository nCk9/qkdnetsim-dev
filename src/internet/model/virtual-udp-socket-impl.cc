...
NS_LOG_COMPONENT_DEFINE ("VirtualUdpSocketImpl");

NS_OBJECT_ENSURE_REGISTERED (VirtualUdpSocketImpl);
...
TypeId
VirtualUdpSocketImpl::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::VirtualUdpSocketImpl")
    .SetParent<UdpSocket> ()
    .SetGroupName ("Internet")
    .AddConstructor<VirtualUdpSocketImpl> ()
    .AddTraceSource ("Drop",
                     "Drop UDP packet due to receive buffer overflow",
                     MakeTraceSourceAccessor (&VirtualUdpSocketImpl::m_dropTrace),
                     "ns3::Packet::TracedCallback")
    .AddAttribute ("IcmpCallback", "Callback invoked whenever an icmp error is received on this socket.",
                   CallbackValue (),
                   MakeCallbackAccessor (&VirtualUdpSocketImpl::m_icmpCallback),
                   MakeCallbackChecker ())
    .AddAttribute ("IcmpCallback6", "Callback invoked whenever an icmpv6 error is received on this socket.",
                   CallbackValue (),
                   MakeCallbackAccessor (&VirtualUdpSocketImpl::m_icmpCallback6),
                   MakeCallbackChecker ())
  ;
  return tid;
}

VirtualUdpSocketImpl::VirtualUdpSocketImpl ()
  : m_endPoint (0),
    m_endPoint6 (0),
    m_node (0),
    m_udp (0),
...
VirtualUdpSocketImpl::~VirtualUdpSocketImpl ()
{
   ...
   * VirtualUdpSocketImpl::Destroy or VirtualUdpSocketImpl::Destroy6
  ...
  if (m_endPoint != 0)
    {
       ...
       * in turn a call to the method VirtualUdpSocketImpl::Destroy below
...
void 
VirtualUdpSocketImpl::SetNode (Ptr<Node> node)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_node = node;
...
void 
VirtualUdpSocketImpl::SetUdp (Ptr<VirtualUdpL4Protocol> udp)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_udp = udp;
}
...
VirtualUdpSocketImpl::GetErrno (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_errno;
}
...
VirtualUdpSocketImpl::GetSocketType (void) const
{
  return NS3_SOCK_DGRAM;
}

Ptr<Node>
VirtualUdpSocketImpl::GetNode (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_node;
}
...
void 
VirtualUdpSocketImpl::Destroy (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_endPoint = 0;
}
...
void
VirtualUdpSocketImpl::Destroy6 (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_endPoint6 = 0;
}
...
void
VirtualUdpSocketImpl::DeallocateEndPoint (void)
{
  if (m_endPoint != 0)
    {
...
int
VirtualUdpSocketImpl::FinishBind (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  bool done = false;
  if (m_endPoint != 0)
    {
      m_endPoint->SetRxCallback (MakeCallback (&VirtualUdpSocketImpl::ForwardUp, Ptr<VirtualUdpSocketImpl> (this)));
      m_endPoint->SetIcmpCallback (MakeCallback (&VirtualUdpSocketImpl::ForwardIcmp, Ptr<VirtualUdpSocketImpl> (this)));
      m_endPoint->SetDestroyCallback (MakeCallback (&VirtualUdpSocketImpl::Destroy, Ptr<VirtualUdpSocketImpl> (this)));
      done = true;
    }
  if (m_endPoint6 != 0)
    {
      m_endPoint6->SetRxCallback (MakeCallback (&VirtualUdpSocketImpl::ForwardUp6, Ptr<VirtualUdpSocketImpl> (this)));
      m_endPoint6->SetIcmpCallback (MakeCallback (&VirtualUdpSocketImpl::ForwardIcmp6, Ptr<VirtualUdpSocketImpl> (this)));
      m_endPoint6->SetDestroyCallback (MakeCallback (&VirtualUdpSocketImpl::Destroy6, Ptr<VirtualUdpSocketImpl> (this)));
      done = true;
    }
  if (done)
    {
...
int
VirtualUdpSocketImpl::Bind (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
int
VirtualUdpSocketImpl::Bind6 (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
int 
VirtualUdpSocketImpl::Bind (const Address &address)
{
...
int 
VirtualUdpSocketImpl::ShutdownSend (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_shutdownSend = true;
  return 0;
...
int 
VirtualUdpSocketImpl::ShutdownRecv (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  m_shutdownRecv = true;
  if (m_endPoint)
...
int
VirtualUdpSocketImpl::Close (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
int
VirtualUdpSocketImpl::Connect (const Address & address)
{
  NS_LOG_FUNCTION (this << address);
...
int 
VirtualUdpSocketImpl::Listen (void)
{
...
int 
VirtualUdpSocketImpl::Send (Ptr<Packet> p, uint32_t flags)
{
...
int 
VirtualUdpSocketImpl::DoSend (Ptr<Packet> p)
{
...
int
VirtualUdpSocketImpl::DoSendTo (Ptr<Packet> p, Ipv4Address dest, uint16_t port, uint8_t tos)
{
  ...
  if (dest.IsBroadcast ())
  ...
  else if (ipv4->GetRoutingProtocol () != 0)
    {
      ...
      header.SetProtocol (VirtualUdpL4Protocol::PROT_NUMBER);
...
int
VirtualUdpSocketImpl::DoSendTo (Ptr<Packet> p, Ipv6Address dest, uint16_t port)
{
  ...
  if (m_endPoint6->GetLocalAddress () != Ipv6Address::GetAny ())
  ...
  else if (ipv6->GetRoutingProtocol () != 0)
    {
      ...
      header.SetNextHeader (VirtualUdpL4Protocol::PROT_NUMBER);
      Socket::SocketErrno errno_;
      Ptr<Ipv6Route> route;
      Ptr<NetDevice> oif = m_boundnetdevice; //specify non-zero if bound to a specific device
      // TBD-- we could cache the route and just check its validity
...
uint32_t
VirtualUdpSocketImpl::GetTxAvailable (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
...
int 
VirtualUdpSocketImpl::SendTo (Ptr<Packet> p, uint32_t flags, const Address &address)
{
...
uint32_t
VirtualUdpSocketImpl::GetRxAvailable (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
...
Ptr<Packet>
VirtualUdpSocketImpl::Recv (uint32_t maxSize, uint32_t flags)
{
...
Ptr<Packet>
VirtualUdpSocketImpl::RecvFrom (uint32_t maxSize, uint32_t flags, 
                         Address &fromAddress)
{
  NS_LOG_FUNCTION (this << maxSize << flags);
...
int
VirtualUdpSocketImpl::GetSockName (Address &address) const
{
  NS_LOG_FUNCTION_NOARGS ();
  if (m_endPoint != 0)
    {
...
int
VirtualUdpSocketImpl::GetPeerName (Address &address) const
{
  NS_LOG_FUNCTION (this << address);
...
int 
VirtualUdpSocketImpl::MulticastJoinGroup (uint32_t interface, const Address &groupAddress)
{
...
int 
VirtualUdpSocketImpl::MulticastLeaveGroup (uint32_t interface, const Address &groupAddress) 
{
  NS_LOG_FUNCTION (interface << groupAddress);
  /*
   1) sanity check interface
...
void
VirtualUdpSocketImpl::BindToNetDevice (Ptr<NetDevice> netdevice)
{
...
void 
VirtualUdpSocketImpl::ForwardUp (Ptr<Packet> packet, Ipv4Header header, uint16_t port,
...
void 
VirtualUdpSocketImpl::ForwardUp6 (Ptr<Packet> packet, Ipv6Header header, uint16_t port, Ptr<Ipv6Interface> incomingInterface)
{
...
void
VirtualUdpSocketImpl::ForwardIcmp (Ipv4Address icmpSource, uint8_t icmpTtl, 
...
void
VirtualUdpSocketImpl::ForwardIcmp6 (Ipv6Address icmpSource, uint8_t icmpTtl, 
                            uint8_t icmpType, uint8_t icmpCode,
                            uint32_t icmpInfo)
{
  NS_LOG_FUNCTION (this << icmpSource << (uint32_t)icmpTtl << (uint32_t)icmpType <<
...
void 
VirtualUdpSocketImpl::SetRcvBufSize (uint32_t size)
{
  m_rcvBufSize = size;
}

uint32_t 
VirtualUdpSocketImpl::GetRcvBufSize (void) const
{
  return m_rcvBufSize;
}

void 
VirtualUdpSocketImpl::SetIpMulticastTtl (uint8_t ipTtl)
{
  m_ipMulticastTtl = ipTtl;
}

uint8_t 
VirtualUdpSocketImpl::GetIpMulticastTtl (void) const
{
  return m_ipMulticastTtl;
}

void 
VirtualUdpSocketImpl::SetIpMulticastIf (int32_t ipIf)
{
  m_ipMulticastIf = ipIf;
}

int32_t 
VirtualUdpSocketImpl::GetIpMulticastIf (void) const
{
  return m_ipMulticastIf;
}

void 
VirtualUdpSocketImpl::SetIpMulticastLoop (bool loop)
{
  m_ipMulticastLoop = loop;
}

bool 
VirtualUdpSocketImpl::GetIpMulticastLoop (void) const
{
  return m_ipMulticastLoop;
}

void 
VirtualUdpSocketImpl::SetMtuDiscover (bool discover)
{
  m_mtuDiscover = discover;
}
bool 
VirtualUdpSocketImpl::GetMtuDiscover (void) const
{
  return m_mtuDiscover;
}

bool
VirtualUdpSocketImpl::SetAllowBroadcast (bool allowBroadcast)
{
...
bool
VirtualUdpSocketImpl::GetAllowBroadcast () const
{
  return m_allowBroadcast;
}

void
VirtualUdpSocketImpl::Ipv6JoinGroup (Ipv6Address address, Socket::Ipv6MulticastFilterMode filterMode, std::vector<Ipv6Address> sourceAddresses)
{
