...
int main (int argc, char *argv[])
{
    ...
    // ipv6.SetBase ("10.1.2.0", "255.255.255.0");
    ...
    std::cout << "Source IP address: " << i0i1.GetAddress(i0i1.GetInterfaceIndex(0), 1) << " " << i0i1.GetAddress(i0i1.GetInterfaceIndex(1), 1)<< std::endl;
    // int idx2 = i1i2.GetInterfaceIndex(1);
    std::cout << "Destination IP address: " << i1i2.GetAddress(i1i2.GetInterfaceIndex(1), 1) << std::endl;
    // std :: cout << "Addresses :" << i0i1.GetAddress(i0i1.GetInterfaceIndex(0), 1);
    /* QKD APPs for charing  */
    QKDAppChargingHelper qkdChargingApp("ns3::TcpSocketFactory", i0i1.GetAddress(i0i1.GetInterfaceIndex(0), 1),  i0i1.GetAddress(i0i1.GetInterfaceIndex(1), 1), 3072000);
    ...
    QKDAppChargingHelper qkdChargingApp12("ns3::TcpSocketFactory", i1i2.GetLinkLocalAddress(0),  i1i2.GetLinkLocalAddress(1), 3072000);
    ...
    QKDSinkAppHelper packetSinkHelper ("ns3::UdpSocketFactory", Inet6SocketAddress (Ipv6Address::GetAny (), sinkPort));
    ApplicationContainer sinkApps = packetSinkHelper.Install (n.Get (2));
    sinkApps.Start (Seconds (25.));
    sinkApps.Stop (Seconds (300.));
    
    /* Create source app  */
    Address sinkAddress (Inet6SocketAddress (i1i2.GetLinkLocalAddress(1), sinkPort));
    Address sourceAddress (Inet6SocketAddress (i0i1.GetLinkLocalAddress(0), sinkPort));
    ...
    p2p.EnablePcapAll ("QKDv6_channel_test");  
